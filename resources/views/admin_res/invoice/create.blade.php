...
<script type="text/javascript">
	...
	var Invoice = {
		...
		addItem: function(){
			var html = '<tr class="item"><td class="id"></td><td><select id="" name="product_id[]" class="form-control selectProduct"><option disabled selected value> -- select an option -- </option>@foreach ($products as $product)<option value="{{$product->id}}">{{$product->name}}</option>@endforeach</select></td><td><input type="number" class="quantity" name="quantity[]" step="1" value="1"></td><td><input disabled="disabled" type="number" class="stock" step="1" value=""></td><td><input type="number" class="price" name="price[]" step="0.01" value=""></td><td class="total-price"></td><td><a href="#" class="delete">x</a></td></tr>';
			$('#items').append(html);
		},
